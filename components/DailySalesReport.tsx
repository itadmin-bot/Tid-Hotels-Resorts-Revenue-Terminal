...
const DailySalesReport: React.FC<DailySalesReportProps> = ({ onManage }) => {
  ...
  const stats = {
    ...
    byMethod: {
      [SettlementMethod.POS]: transactions.reduce((acc, t) => {
        const posAmount = (t.payments || []).filter(p => p.method === SettlementMethod.POS).reduce((sum, p) => sum + p.amount, 0);
        return acc + posAmount;
      }, 0),
